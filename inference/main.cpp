...
//     session_options.SetIntraOpNumThreads(6);
//     session_options.SetInterOpNumThreads(4);
//     session_options.SetGraphOptimizationLevel(ORT_ENABLE_EXTENDED); // Enable extended graph optimizations
...
//         std::vector<float> tensor_data(tensor.data_ptr<float>(), tensor.data_ptr<float>() + tensor.numel());
//         int64_t input_tensor_shape[] = {1, tensor.size(0), tensor.size(1)};

//         std::vector<const char*> input_names = {"input.1"};
//         std::vector<const char*> output_names = {"694"}; // Ensuring this matches your model's output name
